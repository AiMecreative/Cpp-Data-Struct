...
Problem::Problem(int size, int shuffleSteps) {
    this->size = size;
...
bool Problem::isValidAction(std::vector<int> &state, Action action) {
    ...
    if (std::any_of(validActions.begin(), validActions.end(),
                    [action](Action a) { return a == action; })) {
        return true;
    }
    return false;
}
...
void Problem::updateInitState() {
    std::vector<int> state = this->goalState;
    for (int i = 0; i < this->shuffleSteps; i++) {
        std::vector<Action> actions = getValidActions(state);
        state = result(state, actions[rand() % actions.size()]);
    }
    this->initState = state;
...
unsigned int Problem::getAgent(std::vector<int> &state) {
